...
def get_real_data(country_name, days=90):
    ...
    try:
        ...
        if not timeline or not population:
            raise Exception("Could not find 'cases' or 'population' in API response.")
        case_counts = list(timeline.values())
        active_cases = []
        for i in range(len(case_counts)):
            if i < 14: active = case_counts[i] * 0.1
            else: active = case_counts[i] - case_counts[i-14]
            active_cases.append(max(0, active))
        return np.array(active_cases), population
    except requests.exceptions.RequestException as e:
        raise Exception(f"API Error for country '{country_name}': {e}")
    except Exception as e:
